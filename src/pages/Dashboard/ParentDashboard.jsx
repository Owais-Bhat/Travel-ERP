...
import { MdSchool, MdAttachMoney, MdEventNote, MdRefresh, MdPayment, MdChat, MdAssessment, MdEventNote as MdPtm } from 'react-icons/md';
...
const quickActions = [
  { label: 'Pay Fees', icon: MdPayment, path: '/fees' },
  { label: 'Message Teacher', icon: MdChat, path: '/communication' },
  { label: 'View Report Card', icon: MdAssessment, path: '/report-cards' },
  { label: 'PTM Scheduler', icon: MdPtm, path: '/ptm' },
];
...
export default function ParentDashboard({ profile }) {
  ...
  const childCount = dashboardData?.totalStudents ?? 0;

  return (
    <div className="p-5 sm:p-6 lg:p-8 space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-[#0E7C7B] text-xs font-extrabold uppercase tracking-[0.18em] mb-2">Parent Portal</p>
          <h1 className="text-3xl sm:text-4xl font-bold text-slate-950 mb-2">{greetingFor(profile)}</h1>
          <p className="text-slate-500 text-sm">
            {todayLong()}
            {childCount > 1 && <span className="ml-2 text-[#0E7C7B] font-semibold">· {childCount} children linked</span>}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        {statsLoading ? (
          ...
          <>
            <StatCard icon={MdSchool} label="Child's Attendance" value={dashboardData?.attendancePercentage ?? 0} format={(v) => `${Math.round(v)}%`} subtitle="today" accent="#16845D" color="bg-emerald-50 text-emerald-700" />
            ...
            <StatCard icon={MdEventNote} label="Upcoming Exams" value={dashboardData?.activeExams ?? 0} subtitle="for their class" accent="#6F5BD7" color="bg-violet-50 text-[#6F5BD7]" />
          </>
        )}
      </div>
